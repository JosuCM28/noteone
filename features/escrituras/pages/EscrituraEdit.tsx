...
import { getEscritura, postEscritura, updateEscritura } from "../action";

type EscrituraEditProps = {
  escritura: Awaited<ReturnType<typeof getEscritura>>;
}
...
export default function EscrituraEdit({ escritura }: EscrituraEditProps) {
  ...
  const onSubmit: SubmitHandler<FormInput> = async (values) => {
    ...
    try {
      await updateEscritura(escritura.id, data);
      toast.success("Escritura actualizada correctamente");
